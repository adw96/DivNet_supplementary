...
sim_alpha_n <- new_simulation(name = "divnet-n",
...
# saveRDS(sim_alpha_n, file="sim_n.RDS")
# sim_alpha_n_methods <- readRDS(file="sim_n.RDS")

my_data_frame <- right_join(sim_alpha_n %>% model %>% as.data.frame, 
                            sim_alpha_n %>% evals %>% as.data.frame, 
                            by = c("name" = "Model"))
full_data_frame <- my_data_frame %>%
  as_tibble %>%
  separate(col=name, sep="/", into = c("model", "beta", "n", "p", 
                                        "q", "sigma_max", "sigma_min")) %>%
  separate(col = n, sep = "_", into = c("n1", "n_char")) %>%
  select(-n1) %>%
...
# write_csv(full_data_frame, path="n_df.csv")

shannon_plot <- ggplot(full_data_frame, 
                       aes(x = n_char, 
                           y = mse_loss_shannon, col = Method)) +
  geom_boxplot(outlier.size=0.1) + 
  xlab(label="n: Number of samples") +
